"""CLI helper to aggregate medical events from entity mentions."""
...
def main() -> None:
    ...
    builder = MedicalFactBuilder(str(config.database_path), extractor=args.extractor)
    processed = builder.run(content_id=args.content_id)
    logger.info(
        "Derived %s medical event(s) from %s.", processed, args.extractor
    )
